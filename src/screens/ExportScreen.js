...
const PRIMARY_HOVER = colors.blue700;
...
export default function ExportScreen({ navigation }) {
  const imageUri =
    'https://lh3.googleusercontent.com/aida-public/AB6AXuBuqs1HTt8qQ9uutcDrz4dM9aa7OwBWaj35CqSzECD8xjXqPIaM7JZAswdEVrK8fGMZOsk9_pNXAiTBmfYCqZgd4uONZu8_BnxzMOSwOvgSznyPwJQS4ZMqPUn-sHBeIZJ05eMTnpp0DsMFUgR0KtNxVm-g4V7xg8BAH4Ota2XlOBYpgaEM1YCMd4LCOdVePh95_EiWLjv5fe7q3HrykR33QVti6uZ93ITrMqRjft-gzIJxcmTWAnaNewpyoG2EmRsOf4peWOQ_yb-5';

  // UI state
  const [format, setFormat] = React.useState('png'); // 'png' | 'jpg'
  const [resolution, setResolution] = React.useState('hd'); // 'original' | 'hd'

  const isHD = resolution === 'hd';

  const onSave = () => {
    // later: render/export pipeline -> save to gallery
    console.log('Save to Photos (later)');
  };

  const onShare = () => {
    // later: Share API
    console.log('Share (later)');
  };

  const onTryPro = () => {
    console.log('Try Pro (later)');
  };
      ...
      <View style={styles.root}>
        ...
        >
          ...
          <View style={styles.previewWrap}>
            ...
            <View style={styles.infoPill}>
              ...
              <Text style={styles.infoPillText}>2.4 MB • Transparent</Text>
            </View>
          </View>
          ...
          <ResolutionCardPro
            ...
            title="HD"
            subtitle="2160 × 2700 px • Best for print"
            onPress={() => setResolution('hd')}
          />
          ...
          {/* tiny note if JPG chosen */}
          {format === 'jpg' ? (
            <Text style={styles.note}>
              JPG exports include a background (no transparency).
            </Text>
          ) : null}

          {/* tiny note if HD chosen */}
          {isHD ? (
            <Text style={styles.note}>
              HD export is marked as Pro in the UI (we’ll connect your paywall later).
            </Text>
          ) : null}
        </ScrollView>
        ...
        <View style={styles.bottomBar}>
          ...
          >
            ...
            <MaterialIcons name="ios-share" size={20} color={PRIMARY} />
          </Pressable>
  ...
  return (
    ...
    >
      ...
      <View style={[styles.formatIcon, active ? styles.formatIconActive : null]}>
        <MaterialIcons name={iconName} size={22} color={active ? PRIMARY : '#9CA3AF'} />
      </View>
...
function ResolutionCardPro({ active, title, subtitle, onPress }) {
  return (
    ...
    >
      ...
      <MaterialIcons name="hd" size={34} color="rgba(37,99,235,0.18)" />
    </Pressable>
  );
}
...
function Radio({ active, strong }) {
  if (!active) {
    return <View style={styles.radioOff} />;
  }
  return <View style={[styles.radioOn, strong && { borderWidth: 6 }]} />;
}

/** simple checkerboard background without libraries */
function Checkerboard({ style, size = 18 }) {
  // Render a grid using rows/columns of squares
  const cols = 40;
  const rows = 40;
  const squares = [];
  const squareBase = { width: size, height: size };
      ...
      squares.push(
        <View
          ...
          style={[
            styles.checkSquare,
            squareBase,
            isDark ? styles.checkSquareDark : styles.checkSquareLight,
          ]}
        />
      );
    }
  ...
  return (
    <View style={[styles.checkerContainer, style]}>{squares}</View>
  );
}
...
const styles = StyleSheet.create({
  ...
  checkBadge: { position: 'absolute', right: 10, top: 10 },
  formatIcon: {
    width: 44,
    height: 44,
    borderRadius: 12,
